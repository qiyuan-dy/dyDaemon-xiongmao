...
- (void)onLocalFollowTask:(NSNotification *)note {
    ...
    [self followUser:uid secUid:secUid completion:^(BOOL success, NSError *error) {
        ...
        if ([XMGlobalManager sharedInstance].isRunning) {
            if (taskService && [taskService respondsToSelector:@selector(sharedInstance)]) {
                id service = [taskService sharedInstance];
...
- (void)onExecuteTask:(NSNotification *)note {
    ...
    if ([type isEqualToString:@"dz"]) {
        ...
        if (secUid || userId) {
            [self followUser:userId secUid:secUid completion:^(BOOL success, NSError *error) {
                submitBlock(success);
            }];
        } else {
            NSLog(@"[熊猫] 关注任务缺少 sec_uid");
            submitBlock(NO);
        }
    }
